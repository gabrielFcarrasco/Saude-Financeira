...
export const SplashScreen = ({ isReady, onFinish }: { isReady: boolean, onFinish: () => void }) => {
  ...
  // ✨ NOVO: 2. Timer de Segurança (Failsafe de 10s)
  useEffect(() => {
    const safetyTimer = setTimeout(() => {
      // Se deu 10s e ainda não finalizou, força a saída!
      if (!finalizandoRef.current) {
        console.warn("⚠️ [Splash] Ejeção forçada! O 'isReady' não ficou true dentro do tempo limite.");
  ...
  // 4. Vigia Oficial do Encerramento
  useEffect(() => {
  ...
  return (
    <>
      ...
      <div style={{
        ...
        {/* TÍTULO */}
        <h2 style={{ margin: '0 0 8px 0', color: 'var(--text-h)', fontSize: '1.8rem', letterSpacing: '-0.5px', fontWeight: 'bold' }}>Hub do Casal</h2>
        
        {/* TEXTO DINÂMICO */}
        <p style={{ margin: '0 0 48px 0', color: 'var(--text)', fontSize: '1rem', fontStyle: 'italic', transition: 'opacity 0.2s ease-in-out', opacity: opacidadeTexto, minHeight: '24px' }}>
